...
export default async function handler(req) {
  try {
    ...
    // [전송량 다이어트 핵심] 안정화 버전 (600px, WebP, Q60)
    ...
    // [핵심 추가] 애니메이션을 포함한 말풍선 렌더링 함수
    const renderBubble = (lines, textConf, animOffset, animDur) => {
      ...
      // SVG 내부 애니메이션 (<g> 태그로 묶어서 위아래로 둥둥 뜨게 함)
      return `
        <g>
          <animateTransform attributeName="transform" type="translate" values="0,${animOffset}; 0,-${animOffset}; 0,${animOffset}" dur="${animDur}s" repeatCount="indefinite" />
    ...
    <svg width="1024" height="2000" viewBox="0 0 1024 2000" xmlns="http://www.w3.org/2000/svg">
      ...
      ${renderBubble(text1Lines, conf.text1, 8, 2.5)}
      
      ${renderBubble(text2Lines, conf.text2, 12, 3)}
    ...
    return new Response(svg.trim(), { 
      headers: { 
        ...
        'Cache-Control': 'public, max-age=3600' // 브라우저 캐싱 유지
      } 
    });
  } catch (e) {
